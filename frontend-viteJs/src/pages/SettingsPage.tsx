...
import {
  ...
  Button, Dialog, DialogTitle, DialogContent, DialogActions, TextField, IconButton, Snackbar, Alert
} from "@mui/material";
...
const SettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    fetchConfigs()
      ...
      .catch((err) => {
        setSnackbar({ open: true, message: err?.message || 'Failed to fetch configs', severity: 'error' });
      });
  }, []);
  ...
  const handleOpenEdit = (config: any) => {
    setDialogMode('edit');
    setFormData({ config_id: config.config_id, path: config.path, value: config.value });
    setOpenDialog(true);
  };
  ...
  const handleSubmit = async () => {
    try {
      if (dialogMode === 'add') {
        ...
        await updateConfig(Number(formData.config_id), { path: formData.path, value: formData.value });
          ...
          <TableBody>
            {configs.map((config: any) => (
              <TableRow key={config.config_id}>
                ...
                <TableCell>{config.updated_at ? new Date(config.updated_at).toLocaleString() : ''}</TableCell>
                <TableCell>
